...
//				}
		...
		//获取所有该指令出库的盘号
		QueryExample qExample = new QueryExample();
		qExample.setProdOutPlan(bslProductInfo.getProdPlanNo());
		PageHelper.startPage(1, 50);
		List<BslHalfProdMakeInfo> lists = bslProductInfoMapper.halfProdMakeInfo(qExample);
		//判断查询出来的总废料重量，是否足够本次入库
		float sumFLWeight = 0f;
		for (BslHalfProdMakeInfo bslHalfProdMakeInfo : lists) {
			sumFLWeight += bslHalfProdMakeInfo.getProdFlWeight();
		}
		//记录本次入库重量
		Float inWeight = bslProductInfo.getProdRelWeight();
		if(sumFLWeight < inWeight){
			throw new BSLException(ErrorCodeInfo.错误类型_状态校验错误, "该指令出库盘号剩余未使用重量小于本次入库总重量，无法入库！");
		}
		...
		BslStockChangeDetail bslStockChangeDetail = new BslStockChangeDetail();
		//记录纵剪带制造信息
		List<BslZjdUseInfo> bslZjdUseInfoLists = new ArrayList<>();
		String returnProdId = "";
		String prodId;
		float bv = 1f;
		...
		for (int i = 0; i < sumNum; i++) {
			...
			float prodMaxW = 0;
			//记录该产品的父级盘号贡献的最大重量
			float prodNedW = relWeight;
			//开始循环使用上级盘号信息
			for (BslHalfProdMakeInfo bslHalfProdMakeInfo : lists) {
				if(bslHalfProdMakeInfo.getProdFlWeight() == 0f){
					continue;
				}
				//说明加上该盘号还不够，该盘会被完全用掉
				if(bslHalfProdMakeInfo.getProdFlWeight() < prodNedW){
					//插入纵剪带制造信息
					BslZjdUseInfo bslZjdUseInfo = new BslZjdUseInfo();
					bslZjdUseInfo.setProdId(prodId);
					bslZjdUseInfo.setProdZjdId(bslHalfProdMakeInfo.getProdId());
					bslZjdUseInfo.setProdUseWeight(bslHalfProdMakeInfo.getProdFlWeight());
					bslZjdUseInfo.setProdPlanId(bslProductInfo.getProdPlanNo());
					bv = ((float)Math.round(bslHalfProdMakeInfo.getProdFlWeight()/relWeight*100))/100;
					bslZjdUseInfo.setProdUseBl(bv);
					bslZjdUseInfoLists.add(bslZjdUseInfo);

					//记录该产品的父级盘号贡献的最大重量
					if(bslHalfProdMakeInfo.getProdFlWeight() > prodMaxW){
						prodMaxW = bslHalfProdMakeInfo.getProdFlWeight();
						parentProdId = bslHalfProdMakeInfo.getProdId();
					}
					
					//更新还需要的重量
					prodNedW = prodNedW - bslHalfProdMakeInfo.getProdFlWeight();

					//更新该盘剩余重量为0
					bslHalfProdMakeInfo.setProdFlWeight(0f);
					...
					bslZjdUseInfo.setProdId(prodId);
					bslZjdUseInfo.setProdZjdId(bslHalfProdMakeInfo.getProdId());
					bslZjdUseInfo.setProdUseWeight(prodNedW);
					bslZjdUseInfo.setProdPlanId(bslProductInfo.getProdPlanNo());
					bv = ((float)Math.round(prodNedW/relWeight*100))/100;
					bslZjdUseInfo.setProdUseBl(bv);
					bslZjdUseInfoLists.add(bslZjdUseInfo);
					
					//记录该产品的父级盘号贡献的最大重量
					if(prodNedW > prodMaxW){
						prodMaxW = prodNedW;
						parentProdId = bslHalfProdMakeInfo.getProdId();
					}
					
					//更新该盘剩余重量
					bslHalfProdMakeInfo.setProdFlWeight(bslHalfProdMakeInfo.getProdFlWeight() - prodNedW);
			...
			//父级盘号
			BslProductInfo parentProd = bslProductInfoMapper.selectByPrimaryKey(parentProdId);
		...
		//完成之后 插入纵剪带制造信息
		for (BslZjdUseInfo bslZjdUseInfo : bslZjdUseInfoLists) {
			bslZjdUseInfoMapper.insert(bslZjdUseInfo);
		}
...
//		}
	...
	public BSLResult delete(String prodId,String user) {
		...
		//删除之后，纵剪带的制造信息也要删除
		BslZjdUseInfoExample bslZUIExample = new BslZjdUseInfoExample();
		com.bsl.pojo.BslZjdUseInfoExample.Criteria criteria = bslZUIExample.createCriteria();
		criteria.andProdIdEqualTo(prodId);
		bslZjdUseInfoMapper.deleteByExample(bslZUIExample);
