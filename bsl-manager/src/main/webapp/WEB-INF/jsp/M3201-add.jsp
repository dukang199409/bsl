...
<div style="padding:10px 10px 10px 10px">
	<form id="otherProdReceiptAddForm" class="itemForm" method="post">
	   <table>
	        <tr>
	...
	<div style="margin-top:10px" align="center">
	    <a href="javascript:void(0)" class="easyui-linkbutton" id="submitM3201AddForm" onclick="submitM3201AddForm()">提交</a>
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="clearM3201AddForm()">重置</a>
	</div>
</div>
<script type="text/javascript">
	
	//提交表单
	function submitM3201AddForm(){
		//记录录入人员信息
		var inputUser = $("#user_id").html();
		$("#bsInputuserM3201").textbox('setValue',inputUser);
		
		//有效性验证
		if(!$('#otherProdReceiptAddForm').form('validate')){
			$.messager.alert('提示','表单还未填写完成!');
			return;
		}
		//ajax的post方式提交表单
		//$("#itemAddForm").serialize()将表单序列号为key-value形式的字符串
		$('#submitM3201AddForm').linkbutton('disable');
		$.post("/otherProdReceipt/addReceipt",$("#otherProdReceiptAddForm").serialize(), function(data){
			if(data.status == 200){
				$.messager.alert('提示','新增外厂成品来料通知单成功!单号为：'+data.data,'info',function(){
					$("#otherProdReceiptAddWindow").window('close');
					searchM3201Form();
				});
			}else{
				$.messager.alert('提示','错误码：'+data.status+',错误信息：'+data.msg);
			}
			$('#submitM3201AddForm').linkbutton('enable');
		});
	}
	
	function clearM3201AddForm(){
		$('#otherProdReceiptAddForm').form('reset');
	}
