...
<div style="pEditByIding:10px 10px 10px 10px">
	<form id="salePlanDoDetailForm" class="itemForm" method="post">
	   <table>
	   		<tr>
	   			<td width="120" align="right">销售出库通知单号:</td>
	            <td width="210" align="right">
	            	<input name="bsId" id="bsIdM5003Detail" class="easyui-textbox" type="text" readonly="readonly" style="width:200px;"></input>
	            </td>
	    ...
	    <table class="easyui-datagrid" id="saleDetailListM5003" title="销售出库详细信息管理"  style="height:300px"
	       data-options="singleSelect:true,rownumbers:true,fitColumns:true,collapsible:true,toolbar:toolbarM5003D">
	    	<thead>
		        <tr>
...
<script type="text/javascript">
	...
	function getM5003DSelectionsIds(){
		var saleDetailList = $("#saleDetailListM5003");
	...
	var toolbarM5003D = [{
        ...
        iconCls:'icon-print',
        handler:function(){
        	var ids = getM5003DSelectionsIds();
        	if(ids.length == 0){
        	...
        	$.messager.confirm('确认','是否需要PDF打印？',function(r){
				if (r){
        			...
        			mapParam.set("planId",$("#bsIdM5003Detail").textbox('getValue'));
	...
	//重新查询
	function serachM5003Detail(){
		var params = {"bsId": $("#bsIdM5003Detail").val()};
		$.post("/doSalePlan/detail",params, function(data){
				if(data.status == 200){
					$('#saleDetailListM5003').datagrid('loadData', data.data);
				} else {
